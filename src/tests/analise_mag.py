...
probability, real_part, vorticity = explore_sim('mag_nonconst_1e-06_0.0001')

grid = probability.grid
spins = [0, 1, 2]
thetas = [i for i in range(0, grid[1], grid[1]//4)]
phis = [i for i in range(0, grid[2], grid[2]//4)]
rhos = [grid[0]//2-1, grid[0]//2]#i for i in range(0, grid[0], grid[0]//5)]
...
for theta in thetas:
    for phi in phis:
        ...
        vorticity.plot_spins_along_axis('rho', theta, phi)
